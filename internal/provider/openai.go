...
func newOpenAICompatibleProvider(apiKeyEnv, baseURL, model string) *OpenAIProvider {
	apiKey := os.Getenv(apiKeyEnv)
	if apiKey == "" {
		panic("请设置 " + apiKeyEnv + " 环境变量")
	}
...
func NewZhipuOpenAIProvider(model string) *OpenAIProvider {
	return newOpenAICompatibleProvider("ZHIPU_API_KEY", "https://api.z.ai/api/paas/v4/", model)
}
